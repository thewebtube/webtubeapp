...
class JSController(webView: WebView, context : Context) {
    ...
    val preferences: SharedPreferences = context.getSharedPreferences("THEME",
        AppCompatActivity.MODE_PRIVATE
    )
    var btnSettingColor : String = "#000000"
    init {

        if ( preferences.getString("COLOR", "DARK") == "DARK") {
            btnSettingColor = "#FFFFFF"
        }
        this.exec("init")
    }
    // if dark mode is enabled,color is white, else color is black
